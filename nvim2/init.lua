...
later(function()
  ...
  require('toggleterm').setup({
    ...
    -- 基本設定
    open_mapping = [[<C-\>]],  -- Ctrl + \ でターミナル切り替え
    hide_numbers = true,       -- 行番号を非表示
    shade_terminals = true,    -- ターミナルを暗くする
    shading_factor = 2,        -- シェーディング強度
    start_in_insert = true,    -- ターミナル開始時にインサートモード
    insert_mappings = true,    -- インサートモードでもキーマップ有効
    terminal_mappings = true,  -- ターミナルモードでもキーマップ有効
    persist_size = true,       -- サイズを記憶
    persist_mode = true,       -- モードを記憶
    direction = 'horizontal',  -- デフォルトの分割方向
    close_on_exit = true,      -- プロセス終了時にターミナルを閉じる
    shell = vim.o.shell,       -- デフォルトシェルを使用
    auto_scroll = true,        -- 自動スクロール
    -- フローティングターミナルの設定
    float_opts = {
      border = 'curved',       -- ボーダースタイル
      width = function()
  ...
  -- ターミナルバッファ作成時に自動でキーマップを設定
  vim.cmd('autocmd! TermOpen term://* lua set_terminal_keymaps()')
  -- 特殊ターミナルの例（lazygit、htop等）
